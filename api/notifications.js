...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='GET'){const rows=await sql`SELECT id,type,title,body,project_id,read_at,created_at FROM notifications WHERE user_id=${userId} ORDER BY created_at DESC LIMIT 100`;return res.status(200).json({data:rows,unread:rows.filter(x=>!x.read_at).length,unreadCount:rows.filter(x=>!x.read_at).length,persistence:'postgresql'});}
    if(req.method==='POST'){const id=req.body?.id;if(req.body?.action==='read'||req.body?.all===true){if(id)await sql`UPDATE notifications SET read_at=NOW() WHERE id=${id} AND user_id=${userId}`;else await sql`UPDATE notifications SET read_at=NOW() WHERE user_id=${userId} AND read_at IS NULL`;return res.status(200).json({ok:true});}return res.status(400).json({error:'unsupported notification action'});}
    res.setHeader('Allow','GET, POST');return res.status(405).json({error:'Method not allowed'});
